...
class QAOACircuit:
    ...
    def apply_cost_hamiltonian(
        self, circuit: QuantumCircuit, gamma: float, cost_matrix: np.ndarray
    ) -> None:
        """
    ...
    def create_qaoa_circuit(
        ...
        for layer in range(self.num_layers):
            ...
            if use_constrained_mixer:
                self.create_constrained_mixer(circuit, beta)
            else:
...
class PortfolioQAOA:
    ...
    def _cost_function(self, params: np.ndarray, cost_matrix: np.ndarray) -> float:
        ...
        """
        # Create QAOA circuit
        circuit = self.qaoa_circuit.create_qaoa_circuit(
            params,
            cost_matrix,
            use_constrained_mixer=(self.constraint_type == "cardinality"),
        )
        ...
        if self.iteration_count % 50 == 0:
            logger.info(
                f"QAOA iteration {self.iteration_count}: cost = {expectation:.6f}"
            )
    ...
    def solve_portfolio_selection(
        ...
        """
        logger.info(
            f"Starting QAOA portfolio optimization for {self.num_assets} assets"
        )
        ...
        if cardinality_constraint is not None:
            ...
            self.constraint_type = "cardinality"
        ...
        if initial_params is None:
            ...
            initial_params = np.random.uniform(
                0, np.pi / 4, size=self.qaoa_circuit.num_params
            )
        ...
        optimal_circuit = self.qaoa_circuit.create_qaoa_circuit(
            result.x, cost_matrix, use_constrained_mixer=use_constrained_mixer
        )
    ...
    def sample_portfolios(
        self, params: np.ndarray, cost_matrix: np.ndarray, num_samples: int = 1000
