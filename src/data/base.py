...
class DataProvider(ABC):
    ...
    async def _check_rate_limit(self) -> None:
        ...
        self._last_request_times = [
            t for t in self._last_request_times if (now - t).total_seconds() < 60
        ]
        ...
        if len(self._last_request_times) >= self.rate_limit_per_minute:
            ...
            if sleep_time > 0:
                logger.warning(
                    f"Rate limit reached for {self.name}, sleeping {sleep_time:.1f}s"
                )
...
class CachedDataProvider(DataProvider):
    ...
    def __init__(
        self, name: str, cache_ttl_seconds: int = 300, rate_limit_per_minute: int = 60
    ):
        """
