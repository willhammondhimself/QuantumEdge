...
class MeanVarianceOptimizer:
    ...
    def optimize_portfolio(
        ...
        if objective == ObjectiveType.MAXIMIZE_SHARPE:
            ...
            objective_func, cvar_constraints = self._create_cvar_objective(
                w, returns_data, cvar_confidence
            )
            ...
            objective_func, sortino_constraints = self._create_sortino_objective(
                w, expected_returns, returns_data
            )
            ...
            objective_func, calmar_constraints = self._create_calmar_objective(
                w, expected_returns, returns_data, lookback_periods
            )
        ...
        if (
            ...
            and constraints.current_weights is not None
        ):
        ...
        if problem.status in ["optimal", "optimal_inaccurate"]:
            ...
            if expected_variance > 0:
                sharpe_ratio = (expected_return - self.risk_free_rate) / np.sqrt(
                    expected_variance
                )
            else:
                sharpe_ratio = 0.0
    ...
    def _create_cvar_objective(self, w, returns_data: np.ndarray, confidence: float):
        ...
        constraints_cvar = [
            u[i] >= -portfolio_returns[i] - var for i in range(n_scenarios)
        ]

        return cp.Minimize(cvar), constraints_cvar

    def _create_sortino_objective(
        self, w, expected_returns: np.ndarray, returns_data: np.ndarray
    ):
        """
    ...
    def _create_calmar_objective(
        ...
        for i in range(1, lookback):
            constraints_cumulative.append(
                cumulative_returns[i]
                == cumulative_returns[i - 1] + portfolio_returns[i]
            )
        ...
        for i in range(1, lookback):
            ...
            constraints_calmar.append(
                drawdowns[i] == running_max[i] - cumulative_returns[i]
            )
        ...
        return (
            cp.Maximize(expected_portfolio_return - penalty * max_drawdown),
            all_constraints,
        )
