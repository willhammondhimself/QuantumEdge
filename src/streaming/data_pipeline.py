...
class DataPipeline:
    ...
    async def start(self):
        ...
        self._tasks.append(
            asyncio.create_task(self._risk_update_loop())
        )
    ...
    def register_portfolio_calculator(self, portfolio_id: str, calculator_func: Callable):
        """
        ...
        while self._running:
            try:
                if not self.portfolio_subscribers:
                    await asyncio.sleep(self.update_interval * 5)  # Less frequent if no subscribers
                    continue
                ...
                await asyncio.sleep(self.update_interval * 5)  # Portfolio updates less frequent
        ...
        while self._running:
            try:
                if not self.risk_subscribers:
                    await asyncio.sleep(self.update_interval * 30)  # Much less frequent if no subscribers
                    continue
                ...
                await asyncio.sleep(self.update_interval * 30)  # Risk updates much less frequent
