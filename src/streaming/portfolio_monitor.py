...
class PortfolioMonitor:
    ...
    async def add_portfolio(self, portfolio: Portfolio):
        ...
        self.data_pipeline.register_portfolio_calculator(
            ...
            self._create_portfolio_calculator(portfolio)
        )
        
        self.data_pipeline.register_risk_calculator(
            portfolio.portfolio_id,
            self._create_risk_calculator(portfolio)
        )
    ...
    async def remove_portfolio(self, portfolio_id: str):
        ...
        if portfolio_id in self.portfolios:
            ...
            rules_to_remove = [
                rule_id for rule_id, rule in self.alert_rules.items()
    ...
    def _create_portfolio_calculator(self, portfolio: Portfolio):
        ...
        async def calculate_portfolio_value(
            ...
            try:
                ...
                change_percent = change / portfolio.initial_value if portfolio.initial_value != 0 else 0.0
                ...
                logger.error(f"Error calculating portfolio value for {portfolio.portfolio_id}: {e}")
                return None
    ...
    def _create_risk_calculator(self, portfolio: Portfolio):
        ...
        async def calculate_risk_metrics(
            ...
            try:
                ...
                # Check cache first
                if portfolio_id in self._risk_calculation_cache:
                    cached_time, cached_update = self._risk_calculation_cache[portfolio_id]
                ...
                excess_return = mean_return - (portfolio.risk_free_rate / 252)  # Daily risk-free rate
    ...
    async def add_alert_rule(self, rule: AlertRule):
        ...
        logger.info(f"Added alert rule: {rule.rule_id} for portfolio {rule.portfolio_id}")
    ...
    async def _check_portfolio_alerts(self, portfolio: Portfolio, update: PortfolioUpdate):
        """Check portfolio-based alert rules."""
        
        current_time = datetime.utcnow()
        
        for rule in self.alert_rules.values():
            if (rule.portfolio_id != portfolio.portfolio_id or 
                not rule.enabled or
                rule.rule_type not in ["portfolio_change"]):
                continue
            ...
            if rule.rule_type == "portfolio_change":
                if rule.condition == "change_up" and update.change_percent >= rule.threshold:
                    should_trigger = True
                    message = f"Portfolio {portfolio.name} increased by {update.change_percent:.2%}"
                elif rule.condition == "change_down" and update.change_percent <= -rule.threshold:
                    should_trigger = True
                    ...
                    should_trigger = True
                    message = f"Portfolio {portfolio.name} value above ${rule.threshold:,.2f}"
                elif rule.condition == "below" and update.total_value <= rule.threshold:
                    should_trigger = True
                    message = f"Portfolio {portfolio.name} value below ${rule.threshold:,.2f}"
    ...
    async def _check_risk_alerts(self, portfolio: Portfolio, update: RiskUpdate):
        ...
        for rule in self.alert_rules.values():
            ...
            if rule.condition == "volatility_above" and update.volatility >= rule.threshold:
                should_trigger = True
                message = f"Portfolio {portfolio.name} volatility {update.volatility:.2%} exceeds threshold"
                severity = "warning"
            elif rule.condition == "drawdown_below" and update.max_drawdown <= -rule.threshold:
                should_trigger = True
    ...
    def get_portfolio_summary(self, portfolio_id: str) -> Optional[Dict[str, Any]]:
        ...
        total_return = (current_value - portfolio.initial_value) / portfolio.initial_value
