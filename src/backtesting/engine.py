...
class BacktestEngine:
        ...
        if frequency == "daily":
            ...
            while current <= end_date:
                if current.month in [1, 4, 7, 10] and (
                    current.day == 1 or (current - timedelta(days=1)).month != current.month
                ):
                    dates.append(current)
                current += timedelta(days=1)
            ...
            for i, current_date in enumerate(trading_dates):
                ...
                if last_rebalance is None:
                    ...
                    if config.rebalance_frequency == "daily":
                        ...
                        should_rebalance = current_datetime.month != last_rebalance.month
                    elif config.rebalance_frequency == "quarterly":
                        current_quarter = (current_datetime.month - 1) // 3
                        last_quarter = (last_rebalance.month - 1) // 3
                        should_rebalance = (
                            current_quarter != last_quarter or 
                            current_datetime.year != last_rebalance.year
                        )
                ...
                weight_row = {symbol: state.weights.get(symbol, 0.0) for symbol in config.symbols}
    ...
    def generate_report(self, results: Union[BacktestResult, Dict[str, BacktestResult]]) -> Dict[str, Any]:
        """
        ...
        for name, result in results.items():
            if result.success and not result.transactions.empty:
                ...
                report['transaction_costs'][name] = {
                    ...
                    'Average Commission': f"${total_commissions/num_transactions:.2f}" if num_transactions > 0 else "N/A"
                }
