...
def calculate_returns(
    prices: Union[pd.Series, pd.DataFrame], method: str = "simple"
...
def calculate_rolling_metrics(
    ...
    rolling_metrics["volatility"] = returns.rolling(window).std() * np.sqrt(
        periods_per_year
    )
...
def calculate_all_metrics(
    ...
    drawdown_series, max_drawdown, max_dd_duration = calculate_drawdown(
        cumulative_returns
    )
    ...
    if benchmark_values is not None:
        ...
        if len(aligned_data) > 1:
            ...
            beta, alpha = calculate_beta_alpha(
                portfolio_aligned, benchmark_aligned, risk_free_rate
            )
...
def compare_strategies(
    ...
    for name, values in strategies.items():
        try:
            performance, risk = calculate_all_metrics(values, benchmark, risk_free_rate)
            ...
            results.append(
                {
                    ...
                    "Beta": (
                        f"{performance.beta:.3f}"
                        if performance.beta is not None
                        else "N/A"
                    ),
                    "Alpha": (
                        f"{performance.alpha:.3f}"
                        if performance.alpha is not None
                        else "N/A"
                    ),
                }
            )
