...
class PortfolioStrategy(ABC):
    ...
    def should_rebalance(
        self, current_date: datetime, last_rebalance: datetime
    ) -> bool:
        """
...
class QAOAStrategy(PortfolioStrategy):
    ...
    def rebalance(
        ...
        **kwargs,
        ...
        """Optimize using QAOA."""
        try:
            # Edge case: single asset -> allocate fully
            if len(self.symbols) == 1:
                return {self.symbols[0]: 1.0}
            # Get recent price data
            end_idx = prices.index.get_loc(current_date)
            start_idx = max(0, end_idx - self.lookback_period + 1)
            ...
            qaoa = PortfolioQAOA(
                num_assets=len(self.symbols), num_layers=self.num_layers
            )
...
class CustomStrategy(PortfolioStrategy):
    ...
    def should_rebalance(
        ...
        if self.should_rebalance_func:
            return self.should_rebalance_func(
                current_date, last_rebalance, **self.kwargs
            )
    ...
    def rebalance(
        ...
        **kwargs,
        ...
        return self.rebalance_func(
            current_date, prices, current_weights, **merged_kwargs
        )
